...
```shell
# working dir:
cd /nfs/users/nfs_s/sd21/lustre118_link/hc/GENOME/FIX_XCHR
...
# run Pilon
#--- working dir:
cd /nfs/users/nfs_s/sd21/lustre118_link/hc/GENOME/POLISH/POLISH_ARROW_HcV4_haplo_contam
...
```shell   
# working dir:
cd /nfs/users/nfs_s/sd21/lustre118_link/hc/GENOME/CIRCOS
...
```R
# set working dir:
setwd("~/lustre118_link/hc/GENOME/POPULATION_DIVERSITY/GENOME_COVERAGE")
...
# run James' synteny checked for number of genes in runs of 2 to 10 genes in a row
for i in {2..10}; do
...
# generate lists of genes in conserved syntenic runs
...
```bash
# working dir:
cd /nfs/users/nfs_s/sd21/lustre118_link/hc/GENOME/GENOME_QC
...
```bash
# working dir:
cd /nfs/users/nfs_s/sd21/lustre118_link/WBP/WBP_GENOMES/BUSCO_ANALYSIS
mkdir CLADE_V
...
# make the BUSCO names as rownames for the matrix
busco_data  <-  busco_data %>% remove_rownames %>% column_to_rownames(var="BUSCO")

# make a heatmap
pdf()
heatmap.2(as.matrix(busco_data),  scale = "none", trace = "none",  density.info = "none")
dev.off()
```
